...
class BaseAgentTeamStore:
    ...
    def _task_from_record(record) -> AgentTeamTask:
        ...
        blocked_by_raw = fields.get("阻塞依赖") or "[]"
        metadata_raw = fields.get("元数据") or "{}"
        try:
        ...
        return AgentTeamTask(
            ...
            subject=fields.get("任务标题", ""),
            description=fields.get("任务说明", ""),
            role=fields.get("角色", ""),
            status=fields.get("状态", TASK_PENDING),
    ...
    def create_task(self, spec: TaskSpec) -> AgentTeamTask:
        record_id = self.base.create_record(self._table("tasks"), {
            ...
            "元数据": json.dumps(spec.metadata, ensure_ascii=False),
        })
        return AgentTeamTask(
            task_id=record_id,
    ...
    def update_task(self, task_id: str, fields: dict) -> AgentTeamTask:
        ...
        if "metadata" in fields:
            base_fields["元数据"] = json.dumps(fields["metadata"], ensure_ascii=False)
        if base_fields:
