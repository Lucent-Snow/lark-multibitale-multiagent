...
| **Agent-Team** | `src/agent_team/` | 控制平面：Leader 拆解 → Worker 并发认领/仲裁/租约/验证门 |
...
│   └── agent-team.md        # Agent-Team 协议设计文档
├── src/
│   ├── main.py              # CLI 入口
...
│   └── agent_team/          # Agent-Team 控制平面
│       ├── contracts.py     # Task、Claim、TaskEdge 协议
│       ├── schemas.py       # 9 张 Base 表结构定义
│       ├── engine.py        # Leader + Worker（认领/重试/验证门）
│       ├── memory_store.py  # 线程安全内存存储
│       ├── base_store.py    # 飞书 Base 存储
...
│   └── test_agent_team.py   # 39 测试（共 51 测试，全部离线 fake）
└── frontend/
...
# Agent-Team
python src/main.py --agent-team-memory-demo            # 内存协议演示
python src/main.py --agent-team-base-demo              # 真实 Base 多进程演示
python src/main.py --agent-team-worker                 # 运行单个 Worker
python src/main.py --agent-team-setup                  # 创建 9 张 Base 表
...
- **Mission Control（任务中心）**: 预置场景一键启动、自定义目标表单、执行参数配置。
...
```bash
python -m unittest discover -s tests    # 51 测试，全部离线
