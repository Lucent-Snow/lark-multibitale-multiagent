...
`python src/main.py --agent-team-setup` 会创建以下表：

| 配置键 | 作用 |
|---|---|
| `team_objectives` | 目标池 |
| `team_workers` | worker 注册、心跳、当前任务 |
| `team_tasks` | 核心任务看板 |
| `team_task_edges` | 任务 ID 依赖图 |
| `team_claims` | 领取竞争记录 |
| `team_messages` | agent 间消息 |
| `team_artifacts` | 任务产物 |
| `team_verifications` | 验证记录 |
| `team_events` | 审计事件 |
...
claimed/in_progress -- lease expired --> pending
```
...
- worker 先写 `team_claims`，再按 `created_at, claim_id` 判定赢家。
- 输掉 claim 的 worker 不允许写产物或完成任务。
- `claimed` / `in_progress` 任务的 `lease_until` 过期后，会回到 `pending` 并释放 owner，供其他 worker 重新领取。

完成规则：

- task 写入 artifact 后还要通过 verification，才能置为 `completed`。
- verification 只接受严格 JSON；格式不合格或出现阻塞性证据缺口会按 FAIL 处理。
...
```bash
python src/main.py --agent-team-demo --workers 5 --agent-team-max-tasks 4
```

单 worker 进程：

```bash
python src/main.py --agent-team-worker --objective-id <record_id> --worker-id researcher-1 --worker-role researcher
```
